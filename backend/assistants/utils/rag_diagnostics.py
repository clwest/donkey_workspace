...
def run_assistant_rag_test(assistant: Assistant) -> Dict[str, object]:
    """Simulate RAG retrieval for all glossary anchors."""
    anchors = SymbolicMemoryAnchor.objects.all().order_by("slug")
    issues: List[str] = []
    for anchor in anchors:
        debug_info = get_rag_chunk_debug(str(assistant.id), anchor.label)
        ...
        log_rag_debug(
            ...
            anchor.label,
            {
                ...
                "fallback_chunk_scores": [debug_info["scores"][c["chunk_id"]] for c in used],
            },
            debug=True,
...
def run_rag_diagnostics() -> List[Dict[str, object]]:
    """Run RAG tests for all assistants."""
    results = []
    for a in Assistant.objects.all():
        res = run_assistant_rag_test(a)
