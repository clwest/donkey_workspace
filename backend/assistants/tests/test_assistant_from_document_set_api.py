...
class AssistantFromDocumentSetAPITest(BaseAPITestCase):
    def setUp(self):
        self.authenticate()
        self.doc1 = Document.objects.create(title="Doc1", content="t1", source_type="url")
        self.doc2 = Document.objects.create(title="Doc2", content="t2", source_type="url")
    ...
    def test_create_assistant(self, mock_embed):
        ...
        self.assertEqual(AssistantThoughtLog.objects.filter(assistant=assistant).count(), 1)
        self.assertTrue(SwarmMemoryEntry.objects.filter(title__icontains="Summon").exists())
