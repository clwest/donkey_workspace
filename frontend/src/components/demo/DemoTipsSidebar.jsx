...
export default function DemoTipsSidebar({ slug }) {
  const [tips, setTips] = useState([]);
  const [collapsed, setCollapsed] = useState(false);

  useEffect(() => {
    if (!slug) return;
    apiFetch(`/assistants/${slug}/demo_tips/`)
      .then((data) => {
      ...
      {!collapsed && (
        <>
          <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            <button className="btn btn-sm btn-outline-secondary" onClick={markAll}>
              Done
            </button>
          </div>
          ...
          <ul className="list-unstyled small mb-0" style={{ maxHeight: "60vh", overflowY: "auto" }}>
            {tips.map((tip) =>
              tip.dismissed ? null : (
