...
class DemoDriftDiagnosisAPITest(BaseAPITestCase):
    def setUp(self):
        self.authenticate()
        ...
        RAGPlaybackLog.objects.create(
            ...
            query="hi",
            memory_context=None,
            chunks=[{"id": "c1", "is_fallback": True, "anchor_match": False, "glossary_score": 0.0}],
            demo_session_id=self.session_id,
        )

    def test_drift_diagnosis(self):
        url = f"/api/assistants/{self.assistant.slug}/demo_session/{self.session_id}/drift_diagnosis/"
