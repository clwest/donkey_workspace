...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        """Repair or verify DocumentProgress records."""
        if options.get("all"):
        ...
        for doc in docs:
            ...
            if options.get("repair"):
                for ch in chunks:
                    ...
                    if (
                        ...
                        and ch.embedding_status != "embedded"
                    ):
            ...
            if not created:
                if options.get("repair"):
                    ...
                    progress.save()
                    self.stdout.write(
                        f"🔧 Repaired: {doc.title} -> {embedded}/{total} embedded"
                    )
                else:
                    self.stdout.write(
                        f"✅ Verified: {doc.title} -> {embedded}/{total} embedded"
                    )
            else:
                self.stdout.write(
                    f"➕ Created progress: {doc.title} -> {embedded}/{total} embedded"
                )

            if options.get("repair"):
                self.stdout.write(
                    f"   ↪️ Fixed statuses: {fixed} updated, {orphaned} orphaned"
                )
