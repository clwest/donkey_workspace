...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        identifier = kwargs["assistant"]
        assistant = resolve_assistant(identifier)
        if not assistant:
            self.stdout.write(self.style.ERROR(f"No assistant found with identifier '{identifier}'"))
            return
        ...
        if not chunks.exists():
            self.stdout.write(
                f"No chunks linked to assistant '{assistant.slug}' (context {memory_context_id})"
            )
            return

        self.stdout.write(
            f"\nChunks for Assistant '{assistant.slug}' [Context ID: {memory_context_id}]\n"
        )
