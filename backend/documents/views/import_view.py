...
class DocumentImportView(APIView):
    ...
    def post(self, request):
        ...
        job = JobStatus.objects.create(status="queued")
        job_id = str(job.job_id)

        if file_obj:
            path = default_storage.save(file_obj.name, file_obj)
            full_path = os.path.join(settings.MEDIA_ROOT, path)
            process_pdf_upload.delay([full_path], job_id=job_id)
        elif url:
            process_url_upload.delay([url], job_id=job_id)
        elif video:
            process_video_upload.delay([video], job_id=job_id)
        else:
